...
public class ClientsControllerIT {
    ...
    public void save_EmptyNameGiven_ShouldReturnError() {
        ...
        // Do Test
        given().body(resource).contentType(ContentType.JSON).when().post("/api/v1/clients").then()
            .statusCode(HttpStatus.BAD_REQUEST.value()).body("[0].message", is("The name must be between 2 and 30 characters."));
    }
    ...
    public void save_EmptyTokenGiven_ShouldReturnError() {
        ...
        // Do Test
        given().body(resource).contentType(ContentType.JSON).when().post("/api/v1/clients").then()
            .statusCode(HttpStatus.BAD_REQUEST.value()).body("[0].message", is("The token must be between 2 and 50 characters."));
    }
    ...
    public void save_InvalidStatusEmailGiven_ShouldReturnError() {
        ...
        // Do Test
        given().body(resource).contentType(ContentType.JSON).when().post("/api/v1/clients").then()
            .statusCode(HttpStatus.BAD_REQUEST.value()).body("[0].message", is("The status must be either 'INACTIVE' or 'ACTIVE'."));
    }
